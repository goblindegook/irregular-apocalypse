...
describe('Month', () => {
  ...
  it('renders month name and year', () => {
    const { getByText } = render(<Month name='' month={12} year={2012} onPeriodChange={ON_CHANGE} />)
  ...
  it('checks periods with checked=true', () => {
    ...
    const { getByLabelText } = render(<Month periods={periods} name={NAME} signature={SIGNATURE} month={8} year={2018} onPeriodChange={ON_CHANGE} />)
  ...
  it('invokes callback when checkbox is unchecked', () => {
    const fn = jest.fn()
    const { getByLabelText } = render(<Month name={NAME} signature={SIGNATURE} month={8} year={2018} onPeriodChange={fn} />)
